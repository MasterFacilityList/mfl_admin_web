(function (angular, _) {
    ...
    angular.module("mfl.setup.options.controllers", [
        "mfl.service_mgmt.services",
        "ui.router",
        "mfl.common.forms"
    ])

    .controller("mfl.setup.controllers.option_list", [angular.noop])
        ...
        function ($scope, $state, $stateParams, $log, wrappers, forms) {
            ...
            $scope.remove = function () {
                wrappers.options.remove($scope.option_id).success(function(){
                    $state.go("setup.srv_option_list",{},{reload:true});
                }).error(function(error){
                    $log.warn(error);
                });
            };
            $scope.cancel = function () {
                $state.go("setup.srv_option_list.option_edit");
            };
        }
    ])
