(function (angular, _) {
    ...
    angular.module("mfl.setup.services.controllers", [
        "mfl.service_mgmt.services",
        "ui.router"
    ])

    .controller("mfl.setup.controllers.service_list", [angular.noop])
    ...
    .controller("mfl.setup.controllers.service_edit",
        ...
        function ($scope, $state, $stateParams, $log, wrappers) {
            ...
            $scope.tabState = function(obj) {
                ...
                $state.go(
                        "setup.srv_service_list.service_edit."+ obj.name,
            ...
            $scope.remove = function () {
                wrappers.services.remove($scope.service_id).success(function(){
                    $state.go("setup.srv_service_list",{},{reload:true});
                }).error(function(error){
                    $scope.alert = error.error;
                });
            };
            $scope.cancel = function () {
                $state.go("setup.srv_service_list.service_edit");
            };
        }
    ])
            ...
            $scope.save = function (frm) {
                ...
                if (! _.isEmpty(changed)) {
                    wrappers.services.update($scope.service_id, changed)
                        .success(function () {
                            $state.go(
                            "setup.srv_service_list.service_edit.options",
                            {"service_id": $scope.service_id},
                            {reload: true}
                            );
                        });
    ...
    .controller("mfl.setup.controllers.service_edit.options",
        ...
        function ($scope, $state, $log, wrappers) {
            ...
            $scope.cancel= function(){
                $state.go("setup.srv_service_list.service_edit.options");
            };
        }
    ])
        ...
        function ($scope, $state, $stateParams, $log, wrappers, multistepService) {
            ...
            $scope.tabState = function (obj) {
                if(obj.active || obj.done || obj.furthest) {
                    $scope.nextState();
                    $state.go(
                        "setup.srv_service_list.service_create."+ obj.name,
            ...
            $scope.save = function (frm) {
                if(!_.isEmpty($state.params.service_id)) {
                    ...
                    if (! _.isEmpty(changes)) {
                        ...
                        .success(function () {
                            $state.go("setup.srv_service_list.service_create.options",
                    ...
                    .success(function (data) {
                        $state.go(
                            "setup.srv_service_list.service_create.options",
